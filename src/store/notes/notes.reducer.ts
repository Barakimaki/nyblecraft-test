...
import {addNote, addTagToFilter, deleteNote, removeTagFromFilter, updateNote} from "./notes.action";
...
const notesReducer = (
    ...
    if (removeTagFromFilter.match(action)) {
        const newTagFilter = [...state.tagFilter].filter(tag => tag !== action.payload)
        return {
            ...state, tagFilter: newTagFilter
        }
    }
